...
// NOT WORKING WITH RESOLUTION LEVELS SPLIT into different files
public class TestOMETIFFExportCompanion {

    static public void main(String... args) throws Exception {
        String path = "C:/Users/nicol/Desktop/ometiff/";//""C:/Users/chiarutt/Desktop/ometiff/"; // to change!!

        testExport(1024,1024,1,1,1,3,
                128,128,
                2,new File(path+"withoutCompanion.tiff"),
                "LZW", false);

        /*testExport(1024,1024,1,1,1,4,
                128,128,
                2,new File(path+"withCompanion.tiff"),
                "LZW", true);*/
    }

    private static String getFileName(File file, int r) {
    ...
    public static void testExport(int sizeX, int sizeY, int sizeZ, int sizeC, int sizeT, int sizeR,
                                  ...
                                  String compression, boolean withCompanion) throws Exception {
        ...
        // Writes metadata to companion file
        /*if (withCompanion) {*/
            DynamicMetadataOptions options = (DynamicMetadataOptions) writer.getMetadataOptions();
            options.set(OMETiffWriter.COMPANION_KEY, FilenameUtils.removeExtension(file.getAbsolutePath()) + ".companion.ome");
        /*}*/

        writer.setWriteSequentially(true); // Setting this to false can be problematic!
        writer.setMetadataRetrieve(omeMeta);
        writer.setBigTiff(true);
        writer.setId(getFileName(file,0));//file.getAbsolutePath());
        writer.setSeries(0);
        ...
        for (int r = 0; r < sizeR; r++) {
            if (r!=0) writer.changeOutputFile(getFileName(file, r));
