...
import ch.epfl.biop.source.exporter.ImagePlusSampler;
import ij.IJ;
import ij.ImagePlus;
...
/**
 * This command is a simplified version of {@link BdvViewToImagePlusCommand} where
...
@Plugin(type = BdvPlaygroundActionCommand.class,
        //menuPath = BdvPgMenus.RootMenu+"View>BDV>BDV - Export Current View As ImagePlus (Basic)",
        menu = {
                @Menu(label = BdvPgMenus.L1),
                @Menu(label = BdvPgMenus.L2),
                @Menu(label = BdvPgMenus.DisplayMenu, weight = BdvPgMenus.DisplayW),
                @Menu(label = BdvPgMenus.BDVMenu, weight = BdvPgMenus.BDVW),
                @Menu(label = "Export", weight = 0.1),
                @Menu(label = "BDV - Export Current View As ImagePlus (Match Window)", weight = 6)
        },
        description = "Exports the current BDV view as ImagePlus with automatic settings")
public class BasicBdvViewToImagePlusCommand<T extends RealType<T>> implements BdvPlaygroundActionCommand {

    private static Logger logger = LoggerFactory.getLogger(BasicBdvViewToImagePlusCommand.class);
    ...
    String capture_name = "Capture_00";
            ...
            try {
                if (pixelType.equals(ARGBType.class) || pixelType.equals(VolatileARGBType.class)) {
                    typeToSources.get(pixelType).forEach(source -> {
                        Task task = null;
                        if (monitor) task = taskService.createTask("Bdv View export:"+ capture_name);
                        try {
    ...
    private SourceAndConverter<?> createModelSource() {
        ...
        double xNorm = BdvViewToImagePlusCommand.getNormTransform(0, at3D);//trans
        at3D.scale(1/xNorm);

        at3D.scale(1./ sampling_xy, 1./ sampling_xy, 1./ sampling_z);
        at3D.translate((xSize/(2* sampling_xy)), (ySize/(2* sampling_xy)), (z_size /(sampling_z)));

        long nPx = (long)(xSize / sampling_xy);
        long nPy = (long)(ySize / sampling_xy);
        long nPz;
        if (sampling_z ==0) {
            nPz = 1;
        } else {
            nPz = 1+(long)(z_size / (sampling_z /2.0)); // TODO : check div by 2
        }
        ...
        return new EmptySourceCreator(capture_name, at3D.inverse(), nPx, nPy, nPz).get();
    }
    ...
    public void matchXYBDVFrame() {
        ...
        this.xSize= BdvViewToImagePlusCommand.distance(ptTopLeft, ptTopRight);
        this.ySize= BdvViewToImagePlusCommand.distance(ptTopLeft, ptBottomLeft);
    }
