 ...
 *
 * See {@link BasicBdvViewToImagePlusCommand} for a more simpler command with
...
@Plugin(type = BdvPlaygroundActionCommand.class,
        //menuPath = BdvPgMenus.RootMenu+"View>BDV>BDV - Export Current View As ImagePlus",
        menu = {
                ...
                @Menu(label = "BDV - Export Current View As ImagePlus", weight = 6)
        },
        description = "Exports sources as ImagePlus with full control over sampling and region")
public class BdvViewToImagePlusCommand implements BdvPlaygroundActionCommand {

    private static final Logger logger = LoggerFactory.getLogger(BdvViewToImagePlusCommand.class);
    ...
    private SourceAndConverter<?> createModelSource() {
        ...
        at3D.translate((size_x /(2* sampling_xy)), (size_y /(2* sampling_xy)), (size_z /(sampling_z)));

        long nPx = (long)(size_x / sampling_xy);
        long nPy = (long)(size_y / sampling_xy);
        long nPz;
        if (sampling_z ==0) {
            nPz = 1;
        } else {
            nPz = 1+(long)(size_z / (sampling_z /2.0)); // TODO : check div by 2
        }
    ...
    public void matchXYBDVFrame() {
        if (match_window) {
            ...
            this.size_x =distance(ptTopLeft, ptTopRight);
            this.size_y =distance(ptTopLeft, ptBottomLeft);
        }
    }
