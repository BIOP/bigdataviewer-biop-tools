 ...
 * - a comma "," separates independent blocks
 * - a semi colon ":" indicates a range, with bounds included, 0:4 will select 0, 1, 2, 3 and 4
 * - two semi colons "::" can serve to indicate a step size. For instance "0:2:4" will select 0,2 and 4
 * the step is added to the left argument. So "0:2:5" will contain 0,2,4 and not 1,3,5
 * - 'end' indicates the index of the last element of the array
 *
 * some arithmetic can be performed on the indexes ( + - / * )
 *
 * We do not allow for element removal, for instance 1:10 \ 5:6 to indicate 1,2,3,4,7,8,9,10 (from 1 to 10 minus 5 and 6)
 * a potential issue is: should we remove all elements 5 and 6 ? or just the last ones ? this could be ambiguous
 *
 *
 */
public class IntRangeParser {
    ...
    public List<Integer> get(int length) throws Exception {
        ...
        for (String block : blocks) {
            ...
            if (params.length==1) {
                ...
                if (step==0) {
                    ...
                    if (step > 0) {
                        for (double index = startInclusive; index <= endInclusive; index += step) {
        ...
        // Range check
        for (Integer i : list) {
            if (i<0) throw new Exception("Invalid negative index ("+i+")"); // this should never happen
            if (i>=length) throw new Exception("Out of bounds index ("+i+") found in expression");
        }
    ...
    public static void main(String... args) throws Exception {
        ...
        TestExpression("0:0.5:9,9",10, maxDisplay); // Throw error : to duplicate last element
        ...
        TestExpression("0:0.5:-1,0:0.5:9",10, maxDisplay); // Throw error : out of bounds
        TestExpression("-1:-0.5:0,9:-0.5:0",10, maxDisplay); // Throw error : out of bounds
